...
export const ContactsTab: React.FC = () => {
  const { currentUser, selectChat, chats, refreshChats } = useChat();
  ...
  const handleSelectContact = async (contact: EnrichedContact) => {
    ...
    try {
      ...
      let targetChat = chats.find(
        (chat) =>
          chat.type === "direct" &&
          chat.members?.some((m) => m.userId === contact.contactUserId)
      );
      ...
      <div className="p-3 border-b border-[var(--border)] shrink-0">
        <div className="relative">
          ...
          <input
            ...
            placeholder="جستجوی مخاطبین با نام، شماره یا نام‌کاربری..."
            className="w-full bg-[var(--list)] text-xs rounded-xl pr-9 pl-3 py-2 border border-[var(--border)] text-[var(--text-primary)] placeholder:text-[var(--text-secondary)] focus:outline-none focus:border-emerald-500/80 transition-all"
          />
        </div>
      </div>
